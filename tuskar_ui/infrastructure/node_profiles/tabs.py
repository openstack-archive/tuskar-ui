...
class ProfileSuggestion(object):
    ...
    def from_node(cls, node):
        return cls(
            ...
            ram=int(node.properties['ram'] / 1024 / 1024),
            disk=int(node.properties['local_disk'] / 1024 / 1024 / 1024),
    ...
    def from_node_profile(cls, node_profile):
        return cls(
            ...
            ram=node_profile.ram,
            disk=node_profile.disk,
            # TODO(rdopieralski) Add architecture when available.
        )
...
class ProfileSuggestionsTab(horizon.tabs.TableTab):
    ...
    def _get_unmatched_nodes(self):
        node_profiles = api.NodeProfile.list(self.request)
        all_nodes = api.Node.list(self.request)
        unmatched_nodes = []
        def _has_match(node_profile, node):
            return (
        ...
        for node in all_nodes:
            for node_profile in node_profiles:
                if _has_match(node_profile, node):
                    break
            else:
                unmatched_nodes.append(node)
        return unmatched_nodes

    def _get_profile_suggestions(self):
        return set(ProfileSuggestion.from_node(node)
                   for node in self._get_unmatched_nodes())

    def get_profile_suggestions_data(self):
        return list(self._get_profile_suggestions())
