...
horizon.d3SingleBarChart = {
  SingleBarChart: function (chartClass, htmlElement) {
    ...
    if ($.isArray(self.singleBarUsed)) {
      ...
      if (!isNaN(self.singleBarLimit) && !isNaN(self.singleBarUsed)) {
        ...
        // If NaN self.data.percentageUsed is 0
    ...
    if (self.jqueryElement.data("single-bar-color-scale-domain")) {
      self.data.colorScaleDomain = self.jqueryElement.data("single-bar-color-scale-domain");
    ...
    if (self.jqueryElement.data("single-bar-color-scale-range")) {
      self.data.colorScaleRange = self.jqueryElement.data("single-bar-color-scale-range");
    } else {
  ...
  BaseComponent: function (data) {
    ...
    self.bkgrnd = "#F2F2F2";
    self.frgrnd = "grey";
    self.colorScaleMax = 25;  // Is this used anywhere else?
    ...
    // set scales
    self.usageColor = d3.scale.linear().domain(data.colorScaleDomain).range(data.colorScaleRange);
  ...
  UsedComponent: function (baseComponent) {
    ...
    self.append = function (bar, tooltip) {
      ...
      var selfComponent = self.baseComponent;
      bar.append("rect").attr("class", "usedbar").attr("y", usedComponent.y).attr("x", usedComponent.x).attr("width", usedComponent.width).attr("height", usedComponent.height)  //.attr("rx", baseComponent.lvlCurve)
                                                                                                                                                                                 //.attr("ry", baseComponent.lvlCurve)
.style("fill", selfComponent.usageColor(selfComponent.percentageUsedValue())).style("stroke", "#bebebe").style("stroke-width", 0).attr("d", baseComponent.percentageUsedValue()).attr("popup-used", baseComponent.tooltipUsedValue()).on("mouseover", function () {
      ...
      }).transition().duration(500).attr(usedComponent.transitionAttr, usedComponent.transitionValue);
    };
  },
  AverageComponent: function (baseComponent) {
    ...
    self.append = function (bar, tooltip) {
      ...
      bar.append("rect").attr("y", averageComponent.y).attr("x", averageComponent.x).attr("class", "average").attr("height", averageComponent.height).attr("width", averageComponent.width).style("fill", "black").on("mouseover", function () {
        tooltip.style("visibility", "visible");
      }).on("mousemove", function (event) {
        tooltip.style("top", event.pageY - 10 + "px").style("left", event.pageX + 10 + "px");
      }).on("mouseout", function () {
        tooltip.style("visibility", "hidden");
      }).transition().duration(500).attr(averageComponent.transitionAttr, averageComponent.transitionValue);
    };
  },
  ...
  appendTooltip: function (tooltip, htmlContent) {
    ...
    return tooltip.style("position", "absolute").style("z-index", "10").style("visibility", "hidden").style("min-width", "100px").style("max-width", "200px").style("min-height", "30px").style("max-height", "150px").style("border", "1px ridge grey").style("padding", "8px").style("padding-top", "5px").style("background-color", "white").html(htmlContent);
  },
  appendUnused: function (bar, baseComponent, tooltipFree) {
    "use strict";

    bar.append("rect").attr("y", 0).attr("width", baseComponent.w).attr("height", baseComponent.h).attr("rx", baseComponent.lvlCurve).attr("ry", baseComponent.lvlCurve).style("fill", baseComponent.bkgrnd).style("stroke", "#e0e0e0").style("stroke-width", 1).on("mouseover", function () {
      tooltipFree.style("visibility", "visible");
    }).on("mousemove", function (event) {
      tooltipFree.style("top", event.pageY - 10 + "px").style("left", event.pageX + 10 + "px");
    }).on("mouseout", function () {
      tooltipFree.style("visibility", "hidden");
    });
  },
  ...
  appendText: function (bar, baseComponent, tooltip) {
    ...
    bar.append("text").text("FREE").attr("y", baseComponent.h / 2).attr("x", 3).attr("dominant-baseline", "middle").attr("font-size", 13).on("mouseover", function () {
      tooltip.style("visibility", "visible");
    }).on("mousemove", function (event) {
      tooltip.style("top", event.pageY - 10 + "px").style("left", event.pageX + 10 + "px");
    }).on("mouseout", function () {
      tooltip.style("visibility", "hidden");
    }).transition().duration(500).attr("x", function () {
  ...
  appendBorder: function (bar) {
    ...
    bar.append("rect").attr("x", 0).attr("y", 0).attr("height", "100%").attr("width", "100%").style("stroke", "#bebebe").style("fill", "none").style("stroke-width", 1);
  },
  //  INIT
  init: function () {
    "use strict";
  ...
  render: function (htmlElement, data) {
    ...
    var bar = barHtml.append("svg:svg").attr("class", "chart").attr("width", baseComponent.w).attr("height", baseComponent.h).style("background-color", "white").append("g");
    var usedComponent;
