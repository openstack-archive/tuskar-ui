...
class OverviewTab(tabs.Tab):
    ...
    def get_context_data(self, request):
        ...
        deployed_nodes_down = api.node.filter_nodes(
            deployed_nodes, power_state__not_in=api.node.POWER_ON_STATES)
        free_nodes_down = api.node.filter_nodes(
            free_nodes, power_state__not_in=api.node.POWER_ON_STATES)

        nodes_down = itertools.chain(deployed_nodes_down, free_nodes_down)
        nodes_up = api.node.filter_nodes(
        ...
        context = {
            ...
            'local_gb': local_gb,
            'nodes_up_len': sum(1 for _node in nodes_up),
            'nodes_down_len': sum(1 for _node in nodes_down),
            'deployed_nodes_len': sum(1 for _node in deployed_nodes),
            'free_nodes_len': sum(1 for _node in free_nodes),
        }
