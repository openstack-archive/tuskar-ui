...
class UndeployInProgressTab(tabs.Tab):
    ...
    def get_context_data(self, request, **kwargs):
        ...
        # we will hack this around, till API can show this information. So it will
        # actually show progress like the total number is 10, or it will show
        # progress of 5%. Ugly, but workable.
        total_num_nodes_count = 10

        try:
            all_resources_count = len(overcloud.all_resources(with_joins=False))
        except heatclient.exc.HTTPNotFound:
            # Immediately after undeploying has started, heat returns this exception
            # so we can take it as kind of init of undeploying.
