...
def overcloud_keystoneclient(request, url, token, admin=False):
    ...
    endpoint = url
    if admin:
        # FIXME(lsmola) we need to get admin url from heat too, not sure
        # how to get it otherwise. Quick fix is to replace 5000 to 35357
        # Or we will create regular 'tuskar 'user in overcloud, and
        # we will use that, instead of admin token. Though the admin
        # url might be still needed.
        endpoint = endpoint.replace("5000", "35357")

    insecure = False
    cacert = None
    LOG.debug("Creating a new keystoneclient connection to %s." % endpoint)
    conn = api_version['client'].Client(token=token,
                                        endpoint=endpoint,
                                        insecure=insecure,
                                        cacert=cacert,
                                        auth_url=endpoint,
...
class Overcloud(base.APIResourceWrapper):
    ...
    def dashboard_urls(self):
        try:
            ...
            # Stack misses the outputs attribute until it is completed
            LOG.debug(e)
            return []
        ...
        client = overcloud_keystoneclient(self._request,
                                          output['output_value'],
                                          self.attributes['AdminToken'],
                                          admin=True)
        ...
        endpoints = [endpoint.adminurl for endpoint
